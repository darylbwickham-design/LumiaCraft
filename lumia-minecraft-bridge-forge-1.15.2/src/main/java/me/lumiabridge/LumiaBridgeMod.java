...
/** Forge 1.15.2 adapter. Protocol and configuration stay compatible with the other Lumia Bridge builds. */
...
public final class LumiaBridgeMod {
    ...
    private JsonObject playerData(ServerPlayerEntity player) { JsonObject data = identity(player.getName().getString(), player.getUUID()); data.addProperty("dimension", String.valueOf(player.level.dimension.getType().getRegistryName())); data.addProperty("x", player.getX()); data.addProperty("y", player.getY()); data.addProperty("z", player.getZ()); return data; }
    ...
    private final class Host implements LegacyBridgeRuntime.Host {
        ...
        public String gameVersion() { return "1.15.2"; }
